...
aline <-
function(w1,w2,sim=FALSE,m1=NULL,m2=NULL,mark=FALSE,alignment=FALSE,feature.weights=NULL){
  ...
  ind<-as.integer(levels(as.factor(c(n1,n2))))
	...
	if (sim==F) {
		b<-apply(cbind(x,x),1,FUN=raw.alignment,feature.weights)         
		c<-apply(cbind(y,y),1,FUN=raw.alignment,feature.weights) 
